...
static int at32x_write_block(struct flash_bank *bank, const uint8_t *buffer, uint32_t address, uint32_t count);
...
{
    struct target *target = bank->target;
    int retval = target_read_u32(target, AT32_PRODUCT_ID_ADDR, product_id);
    if (retval != ERROR_OK)
        return retval;
    return retval;
}
...
static int at32_get_device_info(struct flash_bank *bank)
{
    ...
    else
    {
        ...
        else if ((at32x_info->bank_addr == BANK2_BASE_ADDR || at32x_info->bank_addr == BANK2_BASE_ADDR_4M) && at32x_info->flash_size > 512)
        {
            if (at32x_info->cur_reg_base == 0)
...
static int at32x_write_usd_data(struct flash_bank *bank)
{
    ...
    retval = target_write_u32(target, at32x_get_flash_reg(bank, AT32_FLASH_CTRL_OFFSET), FLASH_USDPRGM | FLASH_USDULKS);
    if (retval != ERROR_OK)
        return retval;
...
static int at32x_write_block(struct flash_bank *bank, const uint8_t *buffer, uint32_t address, uint32_t count)
{
    ...
    // flash write code
    if (target_alloc_working_area(target, sizeof(at32x_flash_write_code), &write_algorithm) != ERROR_OK)
    {
        LOG_WARNING("no working area available, can't do block memory writes");
        return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;
    }

    retval = target_write_buffer(target, write_algorithm->address, sizeof(at32x_flash_write_code), at32x_flash_write_code);
    if (retval != ERROR_OK)
    {
    ...
    while (target_alloc_working_area_try(target, buffer_size, &source) != ERROR_OK)
    {
        ...
        if (buffer_size <= 256)
        {
            ...
            // free the algorithm target_free_working_area(target, write_algorithm);
...
static int at32x_write(struct flash_bank *bank, const uint8_t *buffer, uint32_t offset, uint32_t count)
{
    struct target *target = bank->target;
    ...
    // Duplicate the buffer and use the normal code path with a single block
    // write since it's probably cheaper than to special case the last odd
    // write using discrete accesses.
    if (count & 1)
    {
    ...
    if (retval == ERROR_TARGET_RESOURCE_NOT_AVAILABLE)
    {
        // if block write failed (no sufficient working area), we use normal(slow) single halfword accesses
...
static int at32x_mass_erase(struct flash_bank *bank)
{
    ...
    if (retval != ERROR_OK)
        return retval;
    retval = target_write_u32(target, at32x_get_flash_reg(bank, AT32_FLASH_CTRL_OFFSET), FLASH_BANKERS | FLASH_ERSTR);
    if (retval != ERROR_OK)
        return retval;
